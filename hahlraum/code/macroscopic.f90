...
contains
    ...
    subroutine source_iteration()
        ...
        integer(kind=short) :: loop,loop_max,i,j
        integer(kind=short) :: matrix_type !> 1=general matrix, 2=sparse matrix
        !> general matrix
        ...
        elseif(matrix_type==2)then
            ...
            call loadmatrix_prepare(im_nrow,im_ncol,im_nval,im_ptB,im_ptE,im_row,&
        ...
        do while(.true.)
            ...
            do i=1,face_number
                if(face(i)%group==face1)then
                    ...
                    if(ctr(face(i)%cell_id(1))%coords(1)<0.1)then
                        face(i)%kappa=0
                    else
                        face(i)%kappa=face(i)%kappa*(1.0d0-exp(-dt/implicit_factor*face(i)%absorbtion_scaled))
                    endif
                endif
            enddo
            ...
            elseif(matrix_type==2)then !>sparse case
                !load matrix
                call loadmatrix_sparse(im_nval,im_nrow,im_ncol,im_val,im_row,im_col,&
                                       ex_nval,ex_nrow,ex_ncol,ex_val,ex_row,ex_col)
                ...
                iparm(1)=0
                msglvl=0
                call pardiso(pt,maxfct,mnum,mtype,phase,im_nrow,im_val,im_row,im_col,perm,nrhs,iparm,msglvl,RHS,solution,error)
                call pardiso(pt,maxfct,mnum,mtype,-1,im_nrow,im_val,im_row,im_col,perm,nrhs,iparm,msglvl,RHS,solution,error)
                if(error/=0)then
    ...
    !--------------------------------------------------------
    subroutine loadmatrix_prepare(im_nrow,im_ncol,im_nval,im_ptB,im_ptE,im_row,&
                                  ex_nrow,ex_ncol,ex_nval,ex_ptB,ex_ptE,ex_row)
        ...
        integer(kind=short) :: cell_id,face_id,node_id
        ...
        do i=1,cell_number_inner
            ...
            if(flag_im(i+cell_number_inner)==0)then
                im_nval=im_nval+1
                flag_im(i+cell_number_inner)=im_nval
                im_ptE(i:im_nrow)=im_ptE(i:im_nrow)+1
            endif
            !-----------------------------------------------
            !>diffusive flux
                ...
                if(face(face_id)%cell_id(1)==i)then !inner normal direction
                    ...
                    if(cell_id/=0)then
                        ...
                        if(ctr(cell_id)%group==cell11 .or. ctr(cell_id)%group==cell12 .or.&
                            ...
                            !    dt/ctr(i)%area*face(face_id)%length*face(face_id)%weight(1)*face(face_id)%kappa
                            if(flag_im(cell_id+cell_number_inner)==0)then
                                im_nval=im_nval+1
                                flag_im(cell_id+cell_number_inner)=im_nval
                                im_ptE(i:im_nrow)=im_ptE(i:im_nrow)+1
                            endif
                        else
                            !matrix_explicit(i,cell_id)=matrix_explicit(i,cell_id)+&
                            !    dt/ctr(i)%area*face(face_id)%length*face(face_id)%weight(1)*face(face_id)%kappa
                            if(flag_ex(cell_id+cell_number)==0)then
                                ex_nval=ex_nval+1
                                flag_ex(cell_id+cell_number)=ex_nval
                                ex_ptE(i:im_nrow)=ex_ptE(i:im_nrow)+1
                            endif
                        endif
                    endif
        ...
        do i=cell_number_inner+1,2*cell_number_inner
            ...
            if(flag_ex(i+(cell_number-cell_number_inner))==0)then
                ex_nval=ex_nval+1
                flag_ex(i+(cell_number-cell_number_inner))=ex_nval
                ex_ptE(i:im_nrow)=ex_ptE(i:im_nrow)+1
            endif
            !> source term
    ...
    end subroutine loadmatrix_prepare

    !--------------------------------------------------
    !> load matrix
    !> implicit and explicit entries
    !--------------------------------------------------
    subroutine loadmatrix_sparse(im_nval,im_nrow,im_ncol,im_val,im_row,im_col,& 
                                 ex_nval,ex_nrow,ex_ncol,ex_val,ex_row,ex_col)
        integer(kind=short),intent(in)  :: im_nval,im_nrow,im_ncol
        integer(kind=short),intent(in)  :: ex_nval,ex_nrow,ex_ncol
        integer(kind=short),intent(in)  :: im_row(im_nrow+1)
        integer(kind=short),intent(in)  :: ex_row(ex_nrow+1)
        ...
        integer(kind=short) :: i,j,k
        real(kind=double) :: swap_value,swap_colume
        integer(kind=short) :: sort1,sort2
        ...
        do i=1,cell_number_inner
            ...
            im_val(flag_im(i))=im_val(flag_im(i))+dt*ctr(i)%absorbtion_scaled
            im_col(flag_im(i))=i
            !matrix_implicit(i,i+cell_number_inner)=matrix_implicit(i,i+cell_number_inner)-&
            !    dt*ctr(i)%absorbtion_scaled
            if(flag_im(i+cell_number_inner)==0)then
                im_nval_temp=im_nval_temp+1
                flag_im(i+cell_number_inner)=im_nval_temp
            endif
            im_val(flag_im(i+cell_number_inner))=im_val(flag_im(i+cell_number_inner))-dt*ctr(i)%absorbtion_scaled
            im_col(flag_im(i+cell_number_inner))=i+cell_number_inner
            !-----------------------------------------------
            !>diffusive flux
            !-----------------------------------------------
            do j=1,ctr(i)%face_number
                ...
                if(face(face_id)%cell_id(1)==i)then !inner normal direction
                    ...
                    if(cell_id/=0)then
                        ...
                        im_val(flag_im(i+cell_number_inner))=im_val(flag_im(i+cell_number_inner))+&
                            dt/ctr(i)%area*face(face_id)%length*face(face_id)%weight(1)*face(face_id)%kappa
                        im_col(flag_im(i+cell_number_inner))=i+cell_number_inner
                        if(ctr(cell_id)%group==cell11 .or. ctr(cell_id)%group==cell12 .or.&
                            ctr(cell_id)%group==cell13 .or. ctr(cell_id)%group==cell14)then
                            ...
                            if(flag_im(cell_id+cell_number_inner)==0)then
                                im_nval_temp=im_nval_temp+1
                                flag_im(cell_id+cell_number_inner)=im_nval_temp
                            endif
                            im_val(flag_im(cell_id+cell_number_inner))=im_val(flag_im(cell_id+cell_number_inner))-&
                                dt/ctr(i)%area*face(face_id)%length*face(face_id)%weight(1)*face(face_id)%kappa
                            im_col(flag_im(cell_id+cell_number_inner))=cell_id+cell_number_inner
                        else
                            !matrix_explicit(i,cell_id)=matrix_explicit(i,cell_id)+&
                            !    dt/ctr(i)%area*face(face_id)%length*face(face_id)%weight(1)*face(face_id)%kappa
                            if(flag_ex(cell_id+cell_number)==0)then
                                ex_nval_temp=ex_nval_temp+1
                                flag_ex(cell_id+cell_number)=ex_nval_temp
                            endif
                            ex_val(flag_ex(cell_id+cell_number))=ex_val(flag_ex(cell_id+cell_number))+&
                                dt/ctr(i)%area*face(face_id)%length*face(face_id)%weight(1)*face(face_id)%kappa
                            ex_col(flag_ex(cell_id+cell_number))=cell_id+cell_number
                        endif
                    endif
                    !tangent derivative
                    !> node1-node2
            ...
            !bubble sort: colume increasing order
            do sort1=im_row(i),im_row(i+1)-2
                do sort2=im_row(i),im_row(i+1)-1-(sort1-im_row(i)+1)
                    if(im_col(sort2)>im_col(sort2+1))then
                        swap_colume     = im_col(sort2)
                        im_col(sort2)   = im_col(sort2+1)
                        im_col(sort2+1) = swap_colume
                        swap_value      = im_val(sort2)
                        im_val(sort2)   = im_val(sort2+1)
                        im_val(sort2+1) = swap_value
                    endif
                enddo
            enddo
            do sort1=ex_row(i),ex_row(i+1)-2
                do sort2=ex_row(i),ex_row(i+1)-1-(sort1-ex_row(i)+1)
                    if(ex_col(sort2)>ex_col(sort2+1))then
                        swap_colume     = ex_col(sort2)
                        ex_col(sort2)   = ex_col(sort2+1)
                        ex_col(sort2+1) = swap_colume
                        swap_value      = ex_val(sort2)
                        ex_val(sort2)   = ex_val(sort2+1)
                        ex_val(sort2+1) = swap_value
                    endif
                enddo
            enddo
        enddo
        ...
        do i=cell_number_inner+1,2*cell_number_inner
            ...
            if(flag_ex(i+(cell_number-cell_number_inner))==0)then
                ex_nval_temp=ex_nval_temp+1
                flag_ex(i+(cell_number-cell_number_inner))=ex_nval_temp
            endif
            ex_val(flag_ex(i+(cell_number-cell_number_inner)))=ex_val(flag_ex(i+(cell_number-cell_number_inner)))+1.0d0
            ex_col(flag_ex(i+(cell_number-cell_number_inner)))=i+(cell_number-cell_number_inner)
            !> source term
            !matrix_implicit(i,i)=matrix_implicit(i,i)+dt*ctr(i-cell_number_inner)%absorbtion_scaled*ctr(i-cell_number_inner)%capacity_scaled
            if(flag_im(i)==0)then
                im_nval_temp=im_nval_temp+1
            ...
            enddo
        enddo
    endsubroutine loadmatrix_sparse

    !--------------------------------------------------
    !> load matrix: general matrix
    !> implicit and explicit entries
    !--------------------------------------------------
