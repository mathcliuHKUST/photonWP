...
contains
    ...
    subroutine init_particle()
        ...
        do i=1,cell_number
            ...
            !ctr(i)%particle_number=max(int(ctr(i)%rho*ctr(i)%area*exp(-dt*ctr(i)%absorbtion_scaled)/ref_weight),0)
    ...
    subroutine sample_particle_cell(cell_number,particle_number,rand_num)
        ...
        do k=particle_id_start,particle_id_end
            ...
            particle(k)%tb=0
            !weight
            !particle(k)%weight=ctr(cell_number)%rho*ctr(cell_number)%area*exp(-dt*ctr(cell_number)%absorbtion_scaled)/&
            !    ctr(cell_number)%particle_number
            particle(k)%weight=ctr(cell_number)%rho*ctr(cell_number)%area/&
                ctr(cell_number)%particle_number
            !velocity
            theta=acos(2.0d0*rand_num(k,1)-1)
            phi=2.0d0*pi*rand_num(k,2)
            particle(k)%v(1)=1.0d0 !lightspeed*sin(theta)*cos(phi)
