...
contains
    ...
    subroutine update_flux(cell,cell_id)
        ...
        cell%h=cell%h+(direction(1)*face_local(1)%flux_h+direction(2)*face_local(2)%flux_h+direction(3)*face_local(3)%flux_h)/cell%volume
    ...
    subroutine calc_flux(face)
        ...
        !--------------------------------------------------
        face%flux_h = dt*face%area*vn*face%h
